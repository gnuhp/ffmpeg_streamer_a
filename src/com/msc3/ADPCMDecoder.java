...
public class ADPCMDecoder
{
	...
	/* return the result data size */
	public static int calResultDataSize(byte[] adpcm, int length)
	{
		int iBlockNumber = (length/BLOCKBYTES);
		return (iBlockNumber * BLOCKSAMPLES * 2);
	}
	public static int decode_1(byte[] adpcm, int lengthToDecode, byte[] outData)
	{
		int iBlockNumber = (lengthToDecode/BLOCKBYTES);
	...
	/* assuming outdata[] has enough space for this otherwise--- goto hell*/
	public static int decode(byte[] adpcm,  byte[] outData)
	{
		int iBlockNumber = (adpcm.length/BLOCKBYTES);
